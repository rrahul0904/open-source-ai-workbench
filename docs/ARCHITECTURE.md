...
5. `src/workflows.mjs` — all executable capability workflows and cross-capability orchestration.
6. `src/providers.mjs` — deterministic providers plus optional live provider adapters.
7. `src/security.mjs` — bearer auth, request-size guard, rate limiting and approval checks.
8. `src/storage.mjs` — memory storage plus optional Redis REST durability.
